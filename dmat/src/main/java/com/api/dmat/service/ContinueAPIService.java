...
public class ContinueAPIService {
	...
	// This function help the userto fetch data from respective email
	public ContinueAPIResponseClass getAssesmentdata(String email) {
		...
		if (userList.size() == 0) {
			// It store the the status as null and assessment id.
			Map<String, Object> map = new HashMap<>();
			map.put("Status", null);
			map.put("assessmentID", null);
			response.setMessage("Data for Assessment does not exsist");
			response.setStatus(HttpStatus.BAD_REQUEST);
			response.setStatusCode(HttpStatus.BAD_REQUEST.value());
			response.setContent(map);
			// Unsuccessful log &  response 
